...
% Open the webcam and start recording at 5fps


% Choose "Calibrate" to adjust perspective shift of the camera
% This is done by setting four points to adjust the angle and then cropping
% the image manually. This action only needs to be performed once and can
% be used for the rest of the stream.


% For each image analyse what is going on in the scene - determine what
% state it is in.


% Read the image
perspective_im = imread(['perspective2.jpg']);

[h,w,d] = size(perspective_im);

% Downsample the image to a reasonable computational size
if(h > 1280)
    height_mod = mod(h,1280);
    
    new_height_ratio = (h - height_mod)/1280+1;
end

perspective_im = perspective_im(1:new_height_ratio:h,1:new_height_ratio:w,:);

[tform, crop_rectangle] = calibrate_perspective(perspective_im);

% Correct the perspective of the camera
perspective_correct = imtransform(perspective_im, tform);


[perspective_crop] = imcrop(perspective_correct, crop_rectangle);
...
%imwrite(perspective_crop,['Thomas/cropped_',num2str(ix), '.jpeg']);

for count = 42
% Start program here - apply both tform and crop to each frame
dice_im = imread(['Thomas/cropped_',num2str(count),'.jpeg']);
...
% Print the result and plot the dice on the image.
fprintf("img %d result: %d %d\n", count,first_die_result,second_die_result);
